...
#Introduction
This is a summary of the Region II port sampling program. The objective of the review is to ensure the sampling goals are proportional to landings. The species reviewed and port sampling goals were obtained from a questionnaire developed by Leland Hulbert. Elisa Russ completed the questionnaire on behalf of Region II (Central Region).  
...
To evaluate potential data gaps, landings per fishery were evaluated as the geometric mean and the geometric standard deviation by port for the most recent five years of complete data (2013, 2014, 2015, 2016, 2017) from the CFEC gross earnings file. The geometric standard deviation is a multiplicative factor to express the deviation from the mean (Kirkwood, 1979); the upper and lower limits expressed in each table represent the range of landings by port over the past five years. Landings were evaluated by calendar year, to match the Annual Management Reports and the opening of most of the commercial groundfish seasons. Landings include directed fishery and bycatch fishery landings; fish reported as discarded at sea. Personal use was included in the Region II analysis as these fish are typically sampled if available. Each fishery is described as the species, area, and fishery location (state-waters, parallel, or throughout the exclusive economic zone). The general fisheries described below are Pacific cod, which includes the state-waters and parallel fishery), walleye pollock, sablefish, lingcod, black rockfish, yelloweye rockfish, quillback rockfish, rougheye rockfish, and shortraker rockfish. Data were subset in OceanAK, variables used are available in the source code.
...
pcod <- cod %>% 
    filter(mgmt_area == "H" | mgmt_area == "E") %>% 
...
prop_cod <- prop_cod %>%  
  mutate( target = ifelse(is.na(target), 550, target)) %>% 
  add_tally(pounds) %>% 
...
ggsave(plot = prop_cod_plot, file = paste0("figures/Region_II/prop_pcod.png"))
...
##Proportion of Pollock Landed and Sampled
```{r proportion_pollock_sampled}
pollock <- read.csv("data/Region_II/walleye_pollock.csv") 
...
prop_pollock <- full_join(pollock, pollock_samples, by = c("year", "mgmt_area")) 

#merge landings and sample data
prop_pollock <- prop_pollock %>%  
...
prop_pollock_plot <- ggplot(data = prop_pollock, 
                        aes(x = year, y = proportion, color = mgmt_area, shape = type))+
  geom_point(size = 4)+
  geom_line(lwd = 1)+
  ylim(0,1)+
  xlim(2013,2017)+
  scale_colour_manual(values = colorRampPalette(PAL)(n_distinct(prop_pollock$mgmt_area)))+
  ylab("Proportion\n")+
  xlab("Year\n")+
  ggtitle("Proportion Pollock Landed and Sampled") 
ggsave(plot = prop_pollock_plot, file = paste0("figures/Region_II/prop_pollock.png"))

print(prop_pollock_plot)
```
...
ggsave(plot = prop_sablefish_plot, file = paste0("figures/Region_II/prop_sablefish_samples.png"))
...
The sampling goal for Cook Inlet sablefish is 550 samples, predominately taken in Seward, due to the fishery occurring in the North Gulf District. During the sablefish fishery, there is not a dedicated port sampler stationed in Seward. Therefore, staff from Homer drive to Seward to sample (4 hours each way).  Vessels are required to call in 6 hours prior to landing to provide notice to staff.  Harvest and effort have declined in recent years, however, sampling goals have still been achieved.
...
The reference line at 550 indicates the sampling goal for PWS and CI (jig only) each.
```{r fig.width = 5.5, fig.height = 3.5, fig.align = "center"}
...
The reference line at 550 indicates the sampling goal for CI (directed), PWS samples do not have a sampling goal.  
```{r fig.width = 5.5, fig.height = 3.5, fig.align = "center"}
...
prop_black_rf <- prop_black_rf %>%  
  mutate(species_use = ifelse(is.na(species_use), "black_rockfish", species_use),
         ...
         target = ifelse(mgmt_area == "H", 550, NA)) %>% 
  add_tally(pounds) %>% 
  rename(total_pounds = n) %>% 
  add_tally(samples) %>% 
  rename(total_samp = n) %>% 
...
ggsave(plot = prop_black_plot, file = paste0("figures/Region_II/prop_black_samples.png"))
...
ggsave(plot = prop_yelloweye_plot, file = paste0("figures/Region_II/prop_yelloweye_samples.png"))
...
The sampling goal for quillback rockfish in the Cook Inlet Area and the Prince William Sound Area is 550 samples, indicated by the reference line at 550.
```{r fig.width = 5.5, fig.height = 3.5, fig.align = "center"}
...
#Proprotion of Quillback Rockfish Landed and Sampled
```{r proportion quillback}
quillback_prop <- quillback %>% 
    filter(mgmt_area == "H" | mgmt_area == "E") %>% 
    select(year, pounds, mgmt_area) %>% 
    group_by(year, mgmt_area) %>%
    summarise_all(funs(sum)) %>% 
...
prop_quillback <- full_join(quillback_prop, quillback_samples, by = c("year", "mgmt_area")) 

prop_quillback <- prop_quillback %>%  
  mutate(species_use = ifelse(is.na(species_use), "quillback", species_use),
         samples = ifelse(is.na(samples), 0, samples),
         target = ifelse(is.na(target), 550, target)) %>% 
  add_tally(pounds) %>% 
  rename(total_pounds = n) %>% 
...
# kable(prop_quillback)
 
prop_quillback_plot <- ggplot(data = prop_quillback,
                        aes(x = year, y = proportion, color = mgmt_area,shape = type))+
 geom_point(size = 4)+
  geom_line(size = 1)+
  ylim(0, 1)+
  xlim(2013, 2017)+
  scale_colour_manual(values = colorRampPalette(PAL)(n_distinct(prop_quillback$mgmt_area)))+
  ylab("Proportion Sampled\n")+
  xlab("Year\n")+
  ggtitle("Proportion Quillback Rockfish Landed and Sampled")
ggsave(plot = prop_quillback_plot, file = paste0("figures/Region_II/prop_quillback_samples.png"))

print(prop_quillback_plot)
```
...
The reference line at 550 indicates the sampling goal for PWS and CI Areas.
```{r fig.width = 5.5, fig.height = 3.5, fig.align = "center"}
...
#Proprotion of Rougheye Rockfish Landed and Sampled
```{r proportion rougheye}
rougheye_prop <- rougheye %>% 
    filter(mgmt_area == "H" | mgmt_area == "E") %>% 
    select(year, pounds, mgmt_area) %>% 
    group_by(year, mgmt_area) %>%
    summarise_all(funs(sum)) %>% 
...
prop_rougheye <- full_join(rougheye_prop, rougheye_samples, by = c("year", "mgmt_area")) 

prop_rougheye <- prop_rougheye %>%  
  mutate(species_use = ifelse(is.na(species_use), "rougheye", species_use),
         samples = ifelse(is.na(samples), 0, samples),
         target = ifelse(is.na(target), 550, target)) %>% 
  add_tally(pounds) %>% 
  rename(total_pounds = n) %>% 
...
prop_rougheye_plot <- ggplot(data = prop_rougheye,
                        aes(x = year, y = proportion, color = mgmt_area,shape = type))+
 geom_point(size = 4)+
  geom_line(size = 1)+
  ylim(0, 1)+
  xlim(2013, 2017)+
  scale_colour_manual(values = colorRampPalette(PAL)(n_distinct(prop_rougheye$mgmt_area)))+
  ylab("Proportion Sampled\n")+
  xlab("Year\n")+
  ggtitle("Proportion Rougheye Rockfish Landed and Sampled")
ggsave(plot = prop_rougheye_plot, file = paste0("figures/Region_II/prop_rougheye_samples.png"))

print(prop_rougheye_plot)
```
...
The reference line at 550 indicates the sampling goal for PWS and CI.
```{r echo = FALSE, fig.width=5.5, fig.height=3.5, fig.align = "center"}
...
##Proprotion of Shortraker Rockfish Landed and Sampled
```{r proportion shortraker}
shortraker_prop <- shortraker %>% 
    filter(mgmt_area == "H" | mgmt_area == "E") %>% 
    select(year, pounds, mgmt_area) %>% 
    group_by(year, mgmt_area) %>%
    summarise_all(funs(sum)) %>% 
...
prop_shortraker <- full_join(shortraker_prop, shortraker_samples, by = c("year", "mgmt_area")) 

prop_shortraker <- prop_shortraker %>%  
  mutate(species_use = ifelse(is.na(species_use), "shortraker", species_use),
         samples = ifelse(is.na(samples), 0, samples),
         target = ifelse(is.na(target), 550, target)) %>% 
  add_tally(pounds) %>% 
  rename(total_pounds = n) %>% 
...
# kable(prop_shortraker)
 
prop_shortraker_plot <- ggplot(data = prop_shortraker,
                        aes(x = year, y = proportion, color = mgmt_area,shape = type))+
 geom_point(size = 4)+
  geom_line(size = 1)+
  ylim(0, 1)+
  xlim(2013, 2017)+
  scale_colour_manual(values = colorRampPalette(PAL)(n_distinct(prop_shortraker$mgmt_area)))+
  ylab("Proportion Sampled\n")+
  xlab("Year\n")+
  ggtitle("Proportion Shortraker Rockfish Landed and Sampled")
ggsave(plot = prop_shortraker_plot, file = paste0("figures/Region_II/prop_shortraker_samples.png"))

print(prop_shortraker_plot)
```
