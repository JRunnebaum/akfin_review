...
```
#Introduction
This is a summary of the Region IV port sampling program. The objective of the review is to ensure the sampling goals are proportional to landings. The species reviewed and port sampling goals were obtained from a questionnaire developed by Leland Hulbert. Natura Richardson completed the questionnaire on behalf of Region IV. The port sampling goals for Region IV were established in 2006 based on Carlile (2005). To reach the defined sampling goals (specified below), the port sampling program  aims to collect samples during the peak of a fishing season to maximize sampling effort within funding constraints. Three seasonal samplers are hired every year, if there is a Tanner crab fishery they are hired for January 15 through April, if there is no Tanner crab fishery they are hired from February through April to sample the height of the state-water Pacific cod fishery.  

State-water Pacific cod is the predominate focus for the Region IV groundfish port sampling program, with one sampler stationed each at Sand Point, King Cove, and Kodiak for the peak of the Pacific cod fishery. The Kodiak and Dutch Harbor state fishery opens seven days after the federal fishery closes. The Chignik fishery opens seven days after the federal fishery closes or March 1st, whichever is later. The South Alaska Peninsula opens 7 days after the federal closure or March 7th, whichever is later. If the federal fishery is opened earlier than what has been typical in the past, then the samplers may miss landings from the Pacific cod state-waters fishery. Samplers at King Cove and Sand Point return to Kodiak after reaching their sampling goals, usually within three weeks. Sampling objectives can be met within this window because of the typically higher catches from pot fisheries that dominate the landings. Samplers are not kept at Sand Point or King Cove for more than 3-weeks, otherwise their salary location would have to be changed from Kodiak to their sampling port, incurring more costs for sampling. Additionally, Sand Point and King Cove each have one processor, while Kodiak has seven, allowing for more species and more processors to be sampled when all three samplers are based in Kodiak. After April samplers usually move to salmon, and groundfish sampling is conducted by Kodiak office staff for the remainder of the year. Port sampling decreases when dedicated technicians are not on call for deliveries are made outside regular working hours or the office is not notified for a delivery, which primarily impacts the rockfish fisheries. 

To evaluate potential data gaps, landings per fishery were evaluated as the geometric mean and the geometric standard deviation by port for the recent most five years of complete data (`r AVGING_YEARS`) from the CFEC gross earnings file. The geometric standard deviation is a multiplicative factor to express the deviation from the mean (Kirkwood, 1979), the upper and lower limits expressed in each figure represent the range of landings by port over the past five years. Landings were evaluated by calendar year to match each region's Annual Management Reporting cycle and the opening of many groundfish fisheries. Landings include directed fishery and bycatch fishery landings, fish reported as discarded at sea or for personal use were removed from all data. Each fishery is described as the species and area. Fisheries were subset by their management type as either a state-water fishery, a parallel fishery, or state management of a fishery throughout the EEZ (0-200 nmi). The general fisheries described below are the state-water Pacific cod, parallel Pacific cod, sablefish, lingcod, black rockfish, and dark rockfish. (Atka mackerel was on the port sampling questionnair but is a fully federally managed fishery and was removed from this analysis). Data were subset in OceanAK by species, selected fields can be found in the source code of this analysis.
...
The Area K sampling goal for Kodiak Pacific cod is 1000 samples, split evenly between the pot and jig fisheries. The sampling goal for Chignik Pacific Cod is 500 samples from the Chignik Area. The sampling goal for South Alaska Peninsula Pacific cod is 500, split between King Cove (250) and Sand Point (250) to capture the spatial extent and predominate ports of landing. No sampling objectives have been set for the Dutch Harbor and Aleutian Islands subdistricts. The reference line at 1000 indicates the sampling goal for the Kodiak Area (K) and the reference line at 500 indicates the sampling goals for Chignik (L) and the South Alaska Peninsula (M) Areas.
...
ggsave(plot = prop_cod_plot, file = paste0("figures/Region_IV/prop_pcod.png"))
...
Dutch Harbor currently does not have a groundfish port sampler, the positions were absorbed into the crab port sampling program prior to crab rationalization in 2005.

The Bering Sea District consists of all state waters of Alaska in the BSAI Area north of the latitude of Cape Sarichef (54° 36′ N lat). The Dutch Harbor Subdistrict (DHS) consists of state waters in the Aleutian Islands District east of 170° W long and state waters of the Bering Sea District west of 164° W long and south of 55° 30′ N lat, except DHS is closed to fishing in Steller sea lion *Eumetopias jubatus* (SSL) protection areas south of 53° 06.11′ N lat.  

The Dutch Harbor Subdistrict state-waters fishery opens with a federal trigger usually late-January to late-February. During the state-waters fishery, the parallel is only closed to pot vessels fishing Pacific cod that are 60 feet or less OAL. The parallel remains open for all other gear types, vessel lengths, and species, even in waters where the state-waters fishery is prosecuted.  

To subset the Dutch Harbor subdistrict, the stat areas between 645410 and 685303 were removed from Area O landings to capture landings in what was defined as the Dutch Harbor subdistrict at the time of landing. The following dates were also used to subset the fishery based on timing. 

2014: 02/11/2014 to 5/31/2014  
2015: 02/09/2015 to 04/04/2015  
2016: 02/12/2016 to 04/24/2016  
2017: 02/09/2017 to 04/11/2017  

```{r eval=FALSE}
Management program code = SMPC      
Stat Area between 645410 and 685303
Regulation management area != K, L, or M
...
pcod_dhbs_sw <- read.csv("data/Region_IV/dutch_cod.csv")

pcod_dhbs_sw <- pcod_dhbs_sw %>% 
  filter(mgmt_prgm_code == "SMPC") %>% 
  mutate(date_landed = mdy(date_landed)) %>% 
  mutate(year = case_when(
          between(date_landed, as.Date("2014-02-11"), as.Date("2014-05-31")) ~ 2014,
          between(date_landed, as.Date("2015-02-09"), as.Date("2015-04-04")) ~ 2015, 
          between(date_landed, as.Date("2016-02-12"), as.Date("2016-04-24")) ~ 2016, 
          between(date_landed, as.Date("2017-02-09"), as.Date("2017-04-11")) ~ 2017)) %>%
  filter(year %in% AVGING_YEARS) %>%
  filter(disposition_code != 95 & disposition_code != 98) %>%
    select(year, port_code, pounds) %>% 
  group_by(year, port_code) %>%
...
The Aleutian Islands District consists of all state waters of Alaska in the BSAI Area south of the latitude of Cape Sarichef (54°36′ N lat). The Aleutian Islands subdistrict is defined as stat areas between 705200 and 999999.

The fishery opens on January 1 in state waters between 175 degrees W and 178 degrees W. The rest of BS state waters are open to parallel fishing during this time. The entire fishery (west of 170 W long) opens with a federal trigger and changes year to year (generally sometime in February or March). When the entire AI is open, there is no parallel Pacific cod fishing. The AI regulations for openings and closures and vessel length restrictions have changed many times over the years, with the most recent change in 2015/16.
...
ggsave(plot = prop_sablefish_plot, file = paste0("figures/Region_IV/prop_sablefish_samples.png"))
...
##Lingcod (0-200 miles)
Lingcod are sampled opportunistically as bycatch in other fisheries and no port sampling goals have been defined. There is a Commissioner's permit fishery for lingcod, but to date there have been no participants.   
...
prop_lingcod <- prop_lingcod %>%  
  mutate(species = ifelse(is.na(species),"lingcod", species),
...
ggsave(plot = prop_lingcod_plot, file = paste0("figures/Region_IV/prop_lingcod_samples.png"))
...
ggsave(plot = prop_black_plot, file = paste0("figures/Region_IV/prop_black_samples.png"))
...
Gaps in sampling occur when landings are stretched out later in the summer and fall when sampler coverage is lower.  
...
The reference line at 500 indicates the sampling goal for Kodak, Chignik, and the South Alaska Peninsula.  

```{r fig.width = 5.5, fig.height = 3.5, fig.align = "center"}
print(dark_samples_plot)
```

#Proportion Dark Rockfish Landed and Sampled
```{r proportion dark rockfish}
dark_rf_prop <- dark_rf %>% 
    filter(mgmt_area == "K" |
           mgmt_area == "L" |
           mgmt_area == "M") %>%
    select(year, pounds, mgmt_area) %>% 
...
prop_dark_rf <- full_join(dark_rf_prop, dark_samples, by = c("year", "mgmt_area")) 

prop_dark_rf <- prop_dark_rf %>%  
  mutate(samples = ifelse(is.na(samples), 0, samples),
         target = ifelse(is.na(target), 500, target)) %>% 
  add_tally(pounds) %>% 
  rename(total_pounds = n) %>% 
  add_tally(samples) %>% 
  rename(total_samp = n) %>% 
...
prop_dark_plot <- ggplot(data = prop_dark_rf,
                        aes(x = year, y = proportion, color = mgmt_area,shape = type))+
  geom_point(size = 4)+
  geom_line(size = 1)+
  ylim(0, 1)+
  xlim(2013, 2017)+
  scale_colour_manual(values = colorRampPalette(PAL)(n_distinct(prop_dark_rf$mgmt_area)))+
  ylab("Proportion Sampled\n")+
  xlab("Year\n")+
  ggtitle("Proportion Dark Rockfish Landed and Sampled")
ggsave(plot = prop_dark_plot, file = paste0("figures/Region_IV/prop_dark_samples.png"))

print(prop_dark_plot)
```


##Dark Rockfish - Kodiak (Area K)
Kodiak dark rockfish are caught as bycatch in other fishery, with no directed fishery, reducing the ability to meet sampling objectives because of the sporadic nature of landings. This area also has low participation in the rockfish fisheries, reducing the likelihood of meeting sampling objectives.
